...
def edit_data():
    ...
    if number_file == 1:
        ...
        # ТУТ НАПИСАТЬ КОД
        name = name_data()  # эта и следующие три строчки я думала запишут новые данные
        surname = surname_data()
        phone = phone_data()
        address = address_data()
        with open(
            "data_first_variant.csv", "w", encoding="utf-8"
        ) as file:  # w - взяла из лекции, я так поняла, что файл должен перезаписаться при использовании этой буквы
            data_first = list(
                file.readlines()
            )  # считала, что создала список из имеющихся записей
            i = (
                number_journal - 1
        ...
        if number_file == 1:
            ...
            with open(
                ...
                if i < len(
                    ...
                    del data_first[
                        i
                    ]  # считала, что это удалит имеющуюся запись с заданным индексом
